    ...
    @foreach ($bought as $key => $item)
        <div class="listing bg-gray-400 dark:bg-gray-800">
            ...
            @if ($item->status == 'COMPLETED')
                <button class="border-2 rounded-3xl px-2 h-12 bg-green-500 text-black transition">Cancelled</button>
            @endif
            </div>
        </div>
        </div>
    ...
    @if($sold->count() > 0)
        ...
        <div class="listing bg-gray-400 dark:bg-gray-800">
            ...
            @if ($item->listing->status == 'reserved' && $item->payment_id && $item->status == 'CREATED')
                ...
                <button class="border-2 hover:border-black rounded-3xl px-2 h-12 bg-amber-300 hover:bg-amber-500 text-black transition">Zarezerwowane</button>
            @endif
            @if ($item->listing->status == 'available' && $item->status == 'CREATED')
            ...
            @endif
            @if ($item->status == 'COMPLETED')
                <button class="border-2 hover:border-black rounded-3xl px-2 h-12 bg-green-500 text-black transition">Cancelled</button>
            @endif
            </div>
        </div>
        </div>
