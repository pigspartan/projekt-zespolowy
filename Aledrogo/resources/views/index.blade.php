<x-layout>
    ...
    <div class="ml-60 mr-60 min-w-min">
        <h1 class="text-center text-3xl p-4 bg-sky-700">List of listed listings</h1>
        @foreach ($listings as $key => $item)
        <div class="p-4 flex {{$key % 2 == 0 ? "bg-indigo-900" : "bg-indigo-950"}}">
            ...
            <div class="flex ml-auto">
                <button onclick="location.href='{{route('itemDetails',['id' => $item->getKey()])}}'" class="border-2 rounded-3xl w-20 h-12 mt-auto mr-2 mb-auto bg-amber-300 hover:bg-amber-500 text-black">BUY</button>
            </div>
        </div>
        @endforeach
